...
public class InventoryService {
    ...
    public Consumer<PurchaseOrder> subscribeOrderStream(){
        return purchaseOrder -> db.computeIfPresent(purchaseOrder.getCategory(), (key,currentQuantity) -> currentQuantity - purchaseOrder.getQuantity());
    }

    public Flux<String> inventoryStream(){
        return Flux.interval(Duration.ofSeconds(2))
                .map((signal) -> db.toString());
    }
}
